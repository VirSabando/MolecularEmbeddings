...
def build_model(params):
    n = params.get('n', 53) # sequence length after padding
    ...
    n_layers = params.get('n_layers', 3) # assume 3 layers
    dense_sizes_lst = params.get('dense_sizes', [256, 64, 16]) # No. of hidden units per dense layer in the model                                                                     
    ...
    if n_layers ==4:
        ...
        y = keras.layers.Dense(1)(drop_d)
    elif n_layers==3:
        # Final Layer if using three dense layers 
        y = keras.layers.Dense(1)(drop_c)
